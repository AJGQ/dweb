...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="obs[count(./preceding-sibling::obs)=0]">
        ...
        }<xsl:if test="not(following-sibling::obs)">]</xsl:if>,
    </xsl:template>

    <xsl:template match="obs[not(count(./preceding-sibling::obs)=0)]">
        {
        "text":[
        <xsl:for-each select="./text()">
            "<xsl:value-of select="normalize-space(.)"/>"<xsl:if test="not(position()=last())">,</xsl:if>
